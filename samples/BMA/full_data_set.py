"""
Mask R-CNN
Train on the modified coco dataset.
...
class FullDataset(utils.Dataset):
    
    def load_full(self, subset, set=None):
        ...
        if subset == "validate":
            print("WARNING! Running 'validate' in method load_full in full_data_set.py")
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = FullDataset()
    dataset_train.load_full(set=args.dataset, subset="train")
    dataset_train.prepare()

    # Development dataset
    dataset_dev = FullDataset()
    dataset_dev.load_full(set=args.dataset, subset="develop")
    dataset_dev.prepare()
